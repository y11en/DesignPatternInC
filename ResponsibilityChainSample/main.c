...
int main(int argc, char **argv)
{
    ...
    //ÔðÈÎÖ÷Ìå
    IApprover *pZhangWj = ApproverDirector2IApprover(ApproverDirector_New(pMemPool, "ÕÅÎÞ¼É"));
    IApprover *pHuangR = ApproverManager2IApprover(ApproverManager_New(pMemPool, "»ÆÈØ"));
    IApprover *pYangG = ApproverVicePresident2IApprover(ApproverVicePresident_New(pMemPool, "Ñî¹ý"));
    IApprover *pGuoJ = ApproverPresident2IApprover(ApproverPresident_New(pMemPool, "¹ù¾¸"));
    IApprover *pBoard = ApproverBoard2IApprover(ApproverBoard_New(pMemPool, "¶­ÊÂ»á"));
